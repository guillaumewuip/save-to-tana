...
export async function askOauth2() {
	...
	console.log(
		"Please open the following URL in your browser to authenticate with Strava:",
	);
	console.log("\n" + oauthUrl + "\n");
	console.log(
...
export async function exchangeCodeForToken(code) {
	if (!STRAVA_CLIENT_ID || !STRAVA_CLIENT_SECRET) {
		throw new Error(
			"STRAVA_CLIENT_ID and STRAVA_CLIENT_SECRET environment variables are not set.",
		);
	}
	...
	if (!response.ok) {
		throw new Error(
			`Failed to exchange code for Strava access token: ${response.status} ${response.statusText}. ${await response.text()}`,
		);
	}
...
export async function waitForOauth2() {
	...
	if (!isAuthenticated()) {
		throw new Error(
			"No Strava tokens found. Please complete OAuth2 authentication first.",
		);
	}
}
...
async function refreshTokens() {
	...
	if (!response.ok) {
		throw new Error(
			`Failed to refresh Strava tokens: ${response.status} ${response.statusText}. ${await response.text()}`,
		);
	}
...
export async function fetchRecentActivities() {
	...
	while (true) {
		Log.debug(
			`Fetching activities (after: ${after}, page: ${page}, per_page: ${per_page})`,
		);
