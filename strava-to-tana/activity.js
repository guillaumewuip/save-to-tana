...
const StravaActivitySchema = z.object({
	...
	name: z.string(),
	sport_type: z
		.enum(["Run", "TrailRun", "Walk", "GravelRide", "Ride", "WeightTraining"])
		.catch("unknown"),
...
export const parseActivities = (data) => {
	...
	if (!result.success) {
		throw new Error(
			`Invalid Strava activity data: ${JSON.stringify(result.error.issues)}`,
		);
	}
...
export const toTanaNode = (activity) => {
	const cadence = activity.average_speed
		? (16.6667 / activity.average_speed).toFixed(2)
		: undefined;

	const node = Tana.Node.createSport(`s-${activity.id}`, {
		name: activity.name,
